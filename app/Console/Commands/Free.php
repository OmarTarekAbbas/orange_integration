...
class Free extends Command
{
    ...
    OrangeSubscribe::where('subscribe_due_date', date("Y-m-d"))->where('free', 1)->chunk(1000, function ($orange_subscribes) {
      foreach ($orange_subscribes as $subscriber) {
        ...
        */
        ...
        $expire_message = "خدمة الفرسان  اللي انت مشترك فيها وبتجيلك كل يوم من أورنج هتتجدد ب 1 جنيه فى اليوم، و لسة عندك الفرصة تكون فائز ب 5000 جنيه يوميًا طوال شهر رمضان لما تحل لغز الفرسان واستهلاك الإنترنت سوف يخصم من الباقة الخاصة بك، ولإلغاء الإشتراك ارسل الغاء إلى  6122مجانا.";
 ...
 // handle old
    OrangeSubscribe::where('subscribe_due_date', "<", date("Y-m-d"))->where('free', 1)->chunk(1000, function ($old_orange_subscribes) {
      foreach ($old_orange_subscribes as $subscriber) {
        // should make subscription request on orange as free 2 days is ended
        $orangeWeb = new Request;
        $orangeWeb->service_id = productId;
        $orangeWeb->msisdn = $subscriber->msisdn;
