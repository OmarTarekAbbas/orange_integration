...
class AdminOrangeController extends Controller
{
    ...
    public function orange_webs(Request $request)
    {
        ...
        if($request->has('time_stamp') && $request->time_stamp != ''){
            $orange_webs = $orange_webs->whereDate('orange_webs.time_stamp',$request->time_stamp);
            $without_paginate = 1;
        }

        if($request->has('on_bearer_type') && $request->on_bearer_type != ''){
            $orange_webs = $orange_webs->whereDate('orange_webs.on_bearer_type',$request->on_bearer_type);
            $without_paginate = 1;
        }
    ...
    {
        $orange_subscribes = OrangeSubscribe::query();
        $without_paginate = 0;
        if($request->has('msisdn') && $request->msisdn != ''){
    ...
    {
        $orange_provisions = Provision::query();
        $without_paginate = 0;
        if($request->has('spId') && $request->spId != ''){
